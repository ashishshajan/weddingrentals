...
$checkStmt = $mysqli->prepare('SELECT id FROM reactiontime_users WHERE id = ? LIMIT 1');
if (!$checkStmt) {
    $mysqli->close();
    http_response_code(500);
...
$stmt = $mysqli->prepare('UPDATE reactiontime_users SET name = ?, emoji = ?, updated_at = NOW() WHERE id = ?');
if (!$stmt) {
    $mysqli->close();
    http_response_code(500);
